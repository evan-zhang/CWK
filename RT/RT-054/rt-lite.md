...
- **受限读取纯本地验收（2026-09-08，本次）**：唯一入口 `make rt054-pure-local` 不接受测试选择或可覆盖解释器；固定外层启动后，runner 以绝对解释器 `-I -S`、固定 bootstrap/argv 与 mode-0700 本地临时 HOME/TMPDIR 执行。子进程只保留 C locale 和 pure-local flag，不继承 PATH、HOME、TMP、Python import/user-site 或任何 NAS/凭据变量；启动期及测试期 socket、`FileStationBackend.from_env`、默认网络 FileStation 写和 `.env` 读取均 fail-closed trap。**93 tests / OK, skipped=1**（21 bounded-read + 50 storage + 16 P0 + 6 pure-local guard）；verbose 摘要明确为 `NasSmokeTests` 的 `SKIP-reason: RT-054 pure-local gate`，completion marker 为四类 trap 均 0。普通父环境与全伪值 parent canary 均经同一入口通过且无伪值泄漏；不读取真实 `.env`。仅 LocalFS、Memory 和 fake FileStation/raw socket 被调用；旧 `read` 与 injected bytes transport 保持不变。静态 guard 证明 gateway/builder/ingest 无 `read_bounded` 接线；未接 P1b、pointer/cache/writer fence/search route，未调用 NAS、生产、真实凭据或真实 payload。P0 safe sink 仍未实现；任何将来的接线须另有独立工作集上限。
...
- 2026-09-08：加固 pure-local 门为固定 Make outer env、不可覆盖的测试集合、绝对解释器 `-I -S` bootstrap、受控临时 HOME/TMPDIR、启动/测试期网络/后端/写/.env trap。两次唯一入口（普通与全伪值父环境）均为 93 tests / OK, skipped=1，四类 trap 均 0；旧 90-test evidence 同样撤销，不声称外部清理确认。P1b 继续 NO-GO。
