...
def dependencies(root,key):
    if key=='a':
        files=[p for base in ('jdk','opensearch/lib','opensearch/plugins','opensearch/modules','opensearch/config','opensearch/bin') for p in (root/base).rglob('*') if p.is_file()]
    else:
        ...
        files+=dictionaries
        files += [p for folder in ('weknora/config','weknora/migrations/sqlite') for p in (root/folder).rglob('*') if p.is_file()]
    return sorted(files)
...
def create(root,window_id,privacy_migration_id):
    ...
    receipt={'schema':'cwk.rt055.ops-freeze.amendment3','run_id':root.name.removeprefix('rt055-'),'window_id':window_id,'privacy_migration_id':privacy_migration_id,'created_at':time.time(),'run_order':['a','b'] if secrets.randbits(1) else ['b','a'],
             ...
             'private_files':{p:ops.sha_file(root/p) for p in ('builder/private-corpus.json','verifier/private-verified.json','verifier/case-verification.json',before_path,status_path,privacy_path)+tuple(window.void_paths(root))+policy_files},'candidates':{},
