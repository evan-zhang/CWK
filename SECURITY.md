...
AI stages are read-only and never use OpenClaw message delivery. Real model calls require a dedicated OpenClaw Agent with `tools.profile=minimal`, only `read` added, and the private `.cwk-ai-runtime` prompt workspace mounted read-only in an Agent-scoped sandbox; general-purpose Agents are rejected at runtime. The model subprocess environment removes CWork and provider secrets, retaining only Gateway authentication when needed. Model prompts are deleted after each call. Manifests record model names and stage status only; they must not contain prompt text, app keys, tokens, or provider credentials.
