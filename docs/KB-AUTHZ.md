...
- **同事从报到到拿令牌都能自己完成（RT-065 / RT-069）。**
  报到：`https://192.168.91.72:8793/register`；签令牌：`https://192.168.91.72:8793/tokens`；
  管理员在 `https://192.168.91.72:8793/members` 给他加库。三步都不用登服务器。
- **同事报到已经可用（RT-065，2026-09-19 上线）。** 同事打开 `https://192.168.91.72:8793/register`，
  填本人工作协同 Key，系统经玄关核实后把人写进人员目录。证书是自签的，首次打开浏览器会告警，
  需要手动继续。管理员也可以继续用命令行的 `enroll`。
  注册只是"系统认识这个人"，**不等于有权限**：没有成员条目，他一个库也读不到。
  下一步要么给他签一支令牌（`kb_token.py issue`），要么等切到成员表判定后直接加成员。
- 建库目前只允许管理员执行。
