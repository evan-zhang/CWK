...
## 架构方向：维护侧与应用侧两半（2026-09-03 定档，RT-039）

CWK 由两半组成，接口是数据、不是代码：

- **维护侧（创建与日常维护）**：采集（3.1 收件箱）→ 入库晋升 → 编译 → AI 精编 → 索引/同步。只写镜像。
- **应用侧（查询）**：`cwk_wiki_query.py`（零依赖维护侧代码，纯标准库）+ `cwk-query` Skill + 镜像内 AGENTS.md 问答契约。只读镜像。
- **边界 = `工作协同镜像/`**（raw + wiki 摘要 + manifest + 索引）。任何一侧重写，只要数据契约不变，另一侧无感——2026-09-03 的存量清理（8858→1539）即实证：查询侧零改动照常工作。

演进两形态：

- **形态 A（当前）**：每人一份私有镜像——各自跑维护侧（自己的收件箱→自己的镜像），Skill 查本地镜像；隐私天然按人隔离。
- **形态 B（多人方向）**：中心化维护服务统一采集/精编，业务 Agent 只装查询 Skill 走远程（查询 HTTP 化 + 按人权限隔离 + 中心机稳定性）。

落地路径：RT-039 查询专用 Skill（薄壳，路由模式）→ 维护侧直连模型 transport（可独立部署；BD-glm 直连已实测可行）→ 出现第二个真实消费者时拆仓（成本极低，因为接口是数据）。
