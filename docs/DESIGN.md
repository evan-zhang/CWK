...
CWK 是一个面向工作协同数据的**只读、Cloud-First 个人知识镜像**。它将授权用户可见的工作汇报转换为：

1. 个人 DocDB 中带版本与 SHA-256 的不可变原文证据；
2. 一篇原文对应一篇可导航摘要；
3. 跨汇报的主题页和实体页；
4. 可回读原文、可验证引文的问答证据包；
5. 每日摘要、事件、实体和运行审计产物；
6. DocDB raw、Wiki、索引与对象清单的持久化主库。
...
- **可追溯**：每个摘要、主题引用和回答都能定位 `report_id`、source SHA-256 与 DocDB file ID。
- **增量**：每日只处理新增、更新、续报和有限历史回填，同时支持完整日期补采。
- **可恢复**：中断后可依据 manifest、磁盘文件和 retry queue 恢复。
- **隐私优先**：raw 只进入授权用户的个人/私有 DocDB；模型调用前隔离敏感内容；不把凭据写入运行产物。
- **低运维**：一条 nightly 命令完成采集、晋级、编译、审计和可选同步。

### 3.2 非目标

- 不回复、审批、驳回、删除、标已读或完成工作协同任务。
- 不把 raw 原文上传到共享/团队知识库；Cloud-First 仅允许个人私有 DocDB。
- 不把模型摘要当作不可质疑的事实源。
...
- `cwk_backfill_range.py`：面向完整性的日期范围分页；验证 API 返回总数与去重后的 ID 数一致。
...
9. 问答默认 `--mode cloud`：从云端索引召回并下载 Top-K raw 验证；缓存命中时必须重新校验 SHA。
10. `--mode shadow` 同时执行本地与云端，迁移期用于比较排序和证据；不得作为静默降级。
