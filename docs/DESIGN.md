...
CWK 是一个面向工作协同数据的**只读个人知识镜像**。它将授权用户可见的工作汇报转换为：
...
- **可追溯**：每个摘要、主题引用和回答都能定位 `report_id` 与本地 raw 路径。
- **增量**：每日只处理新增、更新、续报和有限历史回填，同时支持完整日期补采。
- **可恢复**：中断后可依据 manifest、磁盘文件和 retry queue 恢复。
- **隐私优先**：raw 默认只留本地；模型调用前隔离敏感内容；不把凭据写入运行产物。
...
6. **云端同步不是事实源**：网络失败不影响本地 raw，但会造成云端页面延迟。
7. **配置文档易漂移**：安全策略和模型角色必须以代码预检与 allowlist 为最终约束。

## 14. 关键设计决策

- **ADR-01：raw 本地唯一事实源**——避免云端摘要覆盖原始证据。
