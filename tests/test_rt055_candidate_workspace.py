...
"""
    ...
    def test_native_config_is_copied_without_changes_and_drift_is_rejected(self):
        ...
        migration=self.root/'weknora/migrations/sqlite';migration.mkdir(parents=True);(migration/'0001.sql').write_text('SELECT 1;')
        ...
        self.assertEqual((data/'migrations/sqlite/0001.sql').read_text(),'SELECT 1;')
        (data/'app.db').write_bytes(b'123');(data/'objects').mkdir();(data/'objects/document').write_bytes(b'45')
        self.assertEqual(rt.data_bytes(data,exclude=('config','migrations')),5)
